...
Могут возникнуть ошибки на моменте Prepare environment. Если это происходит нужно проверить файл `.bash_logout` по пути `/home/gitlab-runner` и закомментировать там следующий код, после чего перезапустить пайплайн.
```bash
if [ "$SHLVL" = 1 ]; then
...
Пайплайн мб будет падать, в таком случае посмотреть что все файлы установленные по инструкции из гитлаба принадлежат пользователю `gitlab-runner`, если нет, то прописать `sudo chown -R gitlab-runner .`.
